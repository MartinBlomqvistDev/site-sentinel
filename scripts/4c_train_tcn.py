...
OUTPUT_MODEL_PATH = "models/tcn_master_predictor_tuned.keras"
...
def main():
    ...
    print("--- STEP 4c (ALL-IN): TCN Model Training on Master Dataset ---")
    ...
    print("\n--- TCN (Master) Cross-Validation Results Summary ---")
    print(f"Average Precision: {np.mean(precision_scores):.4f} (+/- {np.std(precision_scores):.4f})")
    print(f"Average Recall:    {np.mean(recall_scores):.4f} (+/- {np.std(recall_scores):.4f})")
    print(f"Average F1-Score:  {np.mean(f1_scores):.4f} (+/- {np.std(f1_scores):.4f})")
    
    print("\nTraining final TCN model on all data for deployment...")
    ...
    print(f"\n✅ Final TCN model saved to '{OUTPUT_MODEL_PATH}'")
