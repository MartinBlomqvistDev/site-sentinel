"""
03b_train_lstm.py — LSTM baseline trained on the master dataset.

Uses 5-fold stratified cross-validation with per-fold class-weight balancing
and a sliding window TimeseriesGenerator (25-frame sequences = ~0.8s history).

The LSTM was included as a baseline to check whether the sequence model could
outperform the Random Forest. It didn't — the rolling features in the tabular
dataset already encode the same temporal context the LSTM was hoping to learn.

Usage:
    python -m pipeline.03b_train_lstm

Input:
    data/analysis_results/master_training_dataset_full.csv
Output:
    models/lstm_master_predictor.keras
"""
...
SEQUENCE_LENGTH: int = _lstm_cfg.get("sequence_length", 25)
EPOCHS: int = _lstm_cfg.get("epochs", 20)
BATCH_SIZE: int = _lstm_cfg.get("batch_size", 32)


def main() -> None:
    logger.info("LSTM baseline training on master dataset")
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X_data, Y_data), start=1):
        ...
        # Compute class weight from the fold's training labels (not global Y_data).
        # Using global labels would inflate the minority weight in later folds.
    ...
    logger.info("Training final LSTM on full dataset...")
    ...
    logger.info("LSTM model saved to %s", output_path)
